...
class MoodSpider(object):
    ...
    def add_chat(self, myMood, user_1, contend_1, user_2, contend_2):
        chat = {}
        chat["user_1"] = user_1
        chat["user_2"] = user_2
        # 处理字符串回去问题 "@{uin:530328538,nick:黄楚亭,who:1,auto:1} 你们这些学习，所以轻松"

        try:
            if contend_1.find('@') != -1:
                contend_1 = contend_1[contend_1.find("}") + 1:]
            if contend_2.find('@') != -1:
                contend_2 = contend_1[contend_2.find("}") + 1:]

            chat["contend_1"] = contend_1
            chat["contend_2"] = contend_2

            # 把字典保存为json格式的字符串
            myMood["chat"].append(chat)
        except Exception, e:
            logging.warning(e)
    ...
    def get_mood(self, texts):
        ...
        # 把评论者的评论提取出来，以对话的形式保存到字典中去，格式如下所示。
        # [{"user1":qq_1,"user_2":qq_2,"comment_1":comment_1,"comment_2":comment_2}] 该列表保存在myMood中


        # 去除头部不属于json格式的数据，在头部加入字符{, 让其满足json格式的数据
        try:
            first_comma_posi = texts.find(',')
            copy_texts = texts[first_comma_posi + 1:-1]
            copy_texts = "{" + copy_texts
            logging.warning(copy_texts)

            # 把json格式的数据以字典的形式保存在变量json_format_text中
            json_format_text = json.loads(copy_texts)
            logging.warning("json 格式正确")
            if json_format_text.has_key("commentlist"):
                myMood["chat"] = []
                # 获取发说说的qq号，发说说的原始类容
                master_qq_num = json_format_text["uin"]
                origin_content = json_format_text["content"]
                logging.warning("=====================")
                # 获取评论者列表
                commentlist = json_format_text["commentlist"]
                cmtnum = json_format_text["cmtnum"]

                # 生成chat 元素
                for guest in commentlist:
                    self.add_chat(myMood, master_qq_num, origin_content, guest["uin"], guest["content"])
                    if guest.has_key("list_3"):
                        list_length = len(guest["list_3"])
                        for i in range(list_length):
                            if i == 0:
                                self.add_chat(myMood, guest["uin"], guest["content"], guest["list_3"][i]["uin"],
                                         guest["list_3"][i]["content"])
                            else:
                                self.add_chat(myMood,guest["list_3"][i]["uin"], guest["list_3"][i]["content"],
                                         guest["list_3"][i - 1]["uin"], guest["list_3"][i - 1]["content"])
                myMood['chat'] = json.dumps(myMood['chat'],encoding='utf-8')
                logging.warning("=====================")
        except Exception,e:
            logging.warning(e)
